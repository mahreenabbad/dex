...
;; (impl-trait 'STXWGJQ101N1C1FYHK64TGTHN4793CHVKRW3ZGVV.sip-010-trait.sip-010-trait)



(define-fungible-token yytoken u1000000000)
...
;;     (try! (ft-transfer? yytoken amount sender recipient))
...
(define-public (transfer
  ...
  (begin
    ...
    (asserts! 
      (is-ok (ft-transfer? yytoken amount sender recipient))
      ERR_TRANSFER_FAILED
    )
...
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance yytoken owner))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin
     ...
     (asserts! (> amount u0) ERR_INVALID_AMOUNT)
    (asserts! (is-eq recipient) ERR_INVALID_RECIPIENT)
		(ft-mint? yytoken amount recipient)
	)
)